 ...
 */
function fillСonditions(objButton)
{
		if (objButton.innerHTML==objButton.value) {
			objButton.innerHTML =objButton.value+" выбрано";
			list_of_ingredients.push(objButton.value)
		}
		else {
			objButton.innerHTML =objButton.value;
			list_of_ingredients = list_of_ingredients.filter(val => val !== objButton.value);
		}
		what_coffee_is()
}
document.addEventListener(onclick,fillСonditions)
...
{
	for (let j = 0; j < Object.keys(types_of_coffee).length; j++)
	{
		if (Object.keys(list_of_ingredients).length === types_of_coffee[j].ingredients.length)
		{
			b = true
			var mass = []
			for (let u = 0; u < types_of_coffee[j].ingredients.length; u++)
			{
				mass.push(types_of_coffee[j].ingredients[u])
			}
			for (let i = 0; i < Object.keys(list_of_ingredients).length; i++) 
			{
				if (typeof(mass.find(element => element == list_of_ingredients[i])) === 'undefined')
				{
					b = false
				}
			}
			if (b === true)
			{
				document.getElementById('coffimg').src=types_of_coffee[j].image;
				document.getElementById('cardheader').innerHTML=types_of_coffee[j].title;
				document.getElementById('cardfooter').innerHTML=types_of_coffee[j].description;
				return 
			}
		}
	}
	document.getElementById('coffimg').src="https://img.freepik.com/premium-photo/empty-mug-on-white_269353-1723.jpg?w=1380";
	document.getElementById('cardheader').innerHTML="По этому рецепту кофе не найдено";
	document.getElementById('cardfooter').innerHTML="Давайте попробуем другие ингридиенты";
	return 
}